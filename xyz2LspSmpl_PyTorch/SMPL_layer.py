...
from smpl_helpers.serialization import ready_arguments
from smpl_helpers import rodrigues_layer
from smpl_helpers.tensutils import (th_posemap_axisang, th_with_zeros, th_pack, make_list)


class SMPL_Layer(Module):
    __constants__ = ['kintree_parents', 'gender', 'center_idx', 'num_joints']

    def __init__(self,gender='neutral',model_root='smpl/native/models',center_idx=None):
        """
        Args:
        ...
        self.faces=torch.Tensor(smpl_data['f'].astype(np.int32))
    ...
    def forward(self,
        ...
        if not self.curr_device:
            self.cur_device = torch.device(th_betas.device.type, th_betas.device.index)
        ...
        return th_verts, th_jtr, self.faces