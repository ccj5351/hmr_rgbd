...
class SMPLRenderer(object):
    ...
    # newly added by CCJ for depth rendering;
    def depth_render(self,
        ...
        dRenderer.camera=ProjectPoints(
            v=verts, 
            rt= np.zeros(3),
            t= np.zeros(3),
            f= cam[0] * np.ones(2),
...
def get_original(proc_param, verts, cam, joints, img_size):
    ...
    print ("[***] undo_scale = {}".format(undo_scale))
    cam_s = cam[0]
    cam_pos = cam[1:]
    principal_pt = np.array([img_size, img_size]) / 2.
    
    flength = 500. # 500. in hmr code;
    #NOTE:??? CCJ: Not sure yet ???
    #flength = 600. # changed to 600 from 500 for rendering SURREAL data;
...
def draw_skeleton(input_image, joints, draw_edges=True, vis=None, radius=None):
    ...
    if joints.shape[1] == 19:
        ...
        parents = np.array([
            ...
            1, 2, 8, 9,3, 4, 7, 8, -1,-1,9,10,13,-1
        ])
        ecolors = {
            0: 'light_pink', # 0: Right ankle
            1: 'light_pink', # 1: Right knee
            2: 'light_pink', # 2: Right hip 
            3: 'pink', # Left hip
            4: 'pink', # Left knee
            5: 'pink', # left ankle
            6: 'light_blue', # right wrist
            7: 'light_blue', # right elbow
            #8: 'light_blue', # not used, due to no parent;
            #9: 'blue', # not used, due to no parent;
            10: 'blue', # left elbow
            11: 'blue', # left wrist
            12: 'purple' # neck
        }
